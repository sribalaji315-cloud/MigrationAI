...
mappings whose value status is 'discontinued'.

For every row in the `workspace_mappings` table where `value_status` equals
'discontinued' and `new_value` is not already 'NOT REQUIRED', set
`new_value` to 'NOT REQUIRED'.

Usage:
    cd backend
    python set_not_required_for_discontinued.py
    python set_not_required_for_discontinued.py --dry-run
"""
...
def run(dry_run: bool = False):
    ...
    try:
        rows = (
            ...
            .filter(
                WorkspaceMapping.value_status == VALUE_STATUS,
        ...
        print(
            f"Found {len(rows)} workspace mapping(s) with "
            f"value_status='{VALUE_STATUS}' and new_value != '{TARGET_VALUE}'."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "whose value status is 'discontinued'."
        )
    )
    parser.add_argument(
        "--dry-run",
